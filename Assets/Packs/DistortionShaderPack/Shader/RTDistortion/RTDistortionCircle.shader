...
Shader "DistortionShaderPack/RTDistortionCircle"
{
	Properties
	{
	...
	SubShader 
	{
		...
		////// Shader functions
			...
			float4 frag(Vert2Frag fragIn) : SV_Target
			{
				...
				float dist = _DistortionSize-min(length(normal), _DistortionSize);
				//float strength = ( (1-abs(normal.x)) * (1-abs(normal.y)) ) * _DistortionStrength;
				//strength = saturate(strength-0.2);
