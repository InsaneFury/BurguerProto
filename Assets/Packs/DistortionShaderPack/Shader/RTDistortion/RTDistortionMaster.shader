...
Shader "DistortionShaderPack/RTDistortionMaster2"
{
	Properties
	{
	...
	SubShader 
	{
		...
		////// Shader functions
			...
			float4 frag(Vert2Frag fragIn) : SV_Target
			{
				...
				//return float4(uvScreen.x, uvScreen.y, 0, 1);
				//return float4(normal.x, normal.y, 0, 1);
				//return float4(direction.x, direction.y, 0, 1);
				//return float4(direction.x+1, direction.y+1, 0, 1)*2 - float4(2,2,0,1);
